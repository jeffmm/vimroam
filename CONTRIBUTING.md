# Contributing to VimRoam

# Filing a bug

Before filing a bug or starting to write a patch, check the latest development version from
https://github.com/jeffmm/vimroam/tree/dev to see if your problem is already fixed.

Issues can be filed at https://github.com/jeffmm/vimroam/issues/ .
...
Make sure to update `doc/vimroam.txt` with the following information:
...
**Testing:** VimRoam uses [vader](https://github.com/junegunn/vader.vim) for unit tests and
...
- In addition to the included tests, there are more example wikis that can be used for testing
  [here](https://github.com/vimroam/testwikis).
...
- Also, when thinking about adding a new feature, it should be something which fits into the
  overall design of VimRoam and which a significant portion of the users may like. Keep in mind
  that everybody has their own way to use VimRoam.
...
- There are two branches with eternal lifetime:
    1. `dev`: This is where the main development happens. Tasks which are done in one or only a few
       ...
       VimRoam unusable (or at least push these commits at one go).
    2. `master`: This branch is for released states only. Whenever a reasonable set of changes has
       ...
       the version number in `plugin/vimroam.vim` is updated. Apart from these commits and the
...
7. Update the version number at the top of plugin/vimroam.vim.
