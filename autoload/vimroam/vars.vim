...
" Description: VimRoam variable definition and manipulation
...
" They consist of:
"
" - global variables. These are stored in the dict g:vimroam_global_vars. They consist mainly of
"   global user variables and syntax stuff which is the same for every syntax.
"
" - wiki-local variables. They are stored in g:vimroam_wikilocal_vars which is a list of
"   dictionaries, one dict for every registered wiki. The last dictionary contains default values
"   (used for temporary wikis).
"
" - syntax variables. Stored in the dict g:vimroam_syntaxlocal_vars which holds all the regexes and
...
" Param: 2: vimroam_key <obj>: user value
...
function! s:check_users_value(key, users_value, value_infos, comes_from_global_variable) abort
  ...
  let setting_origin = a:comes_from_global_variable ?
        \ printf('''g:vimroam_%s''', a:key) :
        \ printf('''%s'' in g:vimroam_list', a:key)
  ...
  if has_key(a:value_infos, 'type') && type(a:users_value) != a:value_infos.type
    call vimroam#u#error(printf('The provided value of the option %s is a %s, ' .
  ...
  if a:value_infos.type == type(0) && has_key(a:value_infos, 'min') &&
    ...
    call vimroam#u#error(printf('The provided value ''%i'' of the option %s is'
  ...
  if has_key(a:value_infos, 'possible_values') &&
    ...
    call vimroam#u#error(printf('The provided value ''%s'' of the option %s is'
  ...
  if a:value_infos.type == type('') && has_key(a:value_infos, 'length') &&
    ...
    call vimroam#u#error(printf('The provided value ''%s'' of the option %s must'
...
function! s:get_default_global() abort
  return {
        ...
        \     'custom:quick']},
        \ 'filetypes': {'type': type([]), 'default': ['markdown']},
        \ 'global_ext': {'type': type(0), 'default': 0, 'min': 0, 'max': 1},
        ...
        \ 'menu': {'type': type(''), 'default': 'VimRoam'},
...
function! s:internal_global_settings() abort
  ...
  let g:vimroam_global_vars.rxSchemes = '\%('.
        \ g:vimroam_global_vars.schemes . '\|'.
        \ g:vimroam_global_vars.web_schemes1 . '\|'.
        \ web_schemes2 .
        \ '\)'
  ...
  let g:vimroam_global_vars.rxWeblinkUrl = rxWebProtocols . '\S\{-1,}'. '\%(([^ \t()]*)\)\='

  let wikilink_prefix = '[['
  let wikilink_suffix = ']]'
  let wikilink_separator = '|'
  let g:vimroam_global_vars.rx_wikilink_prefix = vimroam#u#escape(wikilink_prefix)
  let g:vimroam_global_vars.rx_wikilink_suffix = vimroam#u#escape(wikilink_suffix)
  let g:vimroam_global_vars.rx_wikilink_separator = vimroam#u#escape(wikilink_separator)

  " templates for the creation of wiki links
  " [[URL]]
  let g:vimroam_global_vars.WikiLinkTemplate1 = wikilink_prefix . '__LinkUrl__'. wikilink_suffix
  " [[URL|DESCRIPTION]]
  let g:vimroam_global_vars.WikiLinkTemplate2 = wikilink_prefix . '__LinkUrl__'. wikilink_separator
        \ . '__LinkDescription__' . wikilink_suffix

  let valid_chars = '[^\\\]]'
  let g:vimroam_global_vars.rxWikiLinkUrl = valid_chars.'\{-}'
  let g:vimroam_global_vars.rxWikiLinkDescr = valid_chars.'\{-}'
  ...
  let g:vimroam_global_vars.rxWord = '[^[:blank:]!"$%&''()*+,:;<=>?\[\]\\^`{}]*[^[:blank:]!"$%&''()*+.,:;<=>?\[\]\\^`{}]'

  let g:vimroam_global_vars.rx_wikilink_prefix1 = g:vimroam_global_vars.rx_wikilink_prefix .
        \ g:vimroam_global_vars.rxWikiLinkUrl . g:vimroam_global_vars.rx_wikilink_separator
  let g:vimroam_global_vars.rx_wikilink_suffix1 = g:vimroam_global_vars.rx_wikilink_suffix

  let g:vimroam_global_vars.rxWikiInclPrefix = '{{'
  let g:vimroam_global_vars.rxWikiInclSuffix = '}}'
  let g:vimroam_global_vars.rxWikiInclSeparator = '|'
  " '{{__LinkUrl__}}'
  let g:vimroam_global_vars.WikiInclTemplate1 = g:vimroam_global_vars.rxWikiInclPrefix
        \ .'__LinkUrl__'. g:vimroam_global_vars.rxWikiInclSuffix
  " '{{__LinkUrl____LinkDescription__}}'
  let g:vimroam_global_vars.WikiInclTemplate2 = g:vimroam_global_vars.rxWikiInclPrefix
        \ . '__LinkUrl__' . g:vimroam_global_vars.rxWikiInclSeparator . '__LinkDescription__'
        \ . g:vimroam_global_vars.rxWikiInclSuffix

  let valid_chars = '[^\\\}]'
  let g:vimroam_global_vars.rxWikiInclUrl = valid_chars.'\{-}'
  let g:vimroam_global_vars.rxWikiInclArg = valid_chars.'\{-}'
  let g:vimroam_global_vars.rxWikiInclArgs = '\%('. g:vimroam_global_vars.rxWikiInclSeparator.
        \ g:vimroam_global_vars.rxWikiInclArg. '\)'.'\{-}'

  " *. {{URL}[{...}]}  - i.e.  {{URL}}, {{URL|ARG1}}, {{URL|ARG1|ARG2}}, etc.
  " *a) match {{URL}[{...}]}
  let g:vimroam_global_vars.rxWikiIncl = g:vimroam_global_vars.rxWikiInclPrefix.
        \ g:vimroam_global_vars.rxWikiInclUrl.
        \ g:vimroam_global_vars.rxWikiInclArgs. g:vimroam_global_vars.rxWikiInclSuffix
  " *b) match URL within {{URL}[{...}]}
  let g:vimroam_global_vars.rxWikiInclMatchUrl = g:vimroam_global_vars.rxWikiInclPrefix.
        \ '\zs'. g:vimroam_global_vars.rxWikiInclUrl . '\ze'.
        \ g:vimroam_global_vars.rxWikiInclArgs . g:vimroam_global_vars.rxWikiInclSuffix

  let g:vimroam_global_vars.rxWikiInclPrefix1 = g:vimroam_global_vars.rxWikiInclPrefix.
        \ g:vimroam_global_vars.rxWikiInclUrl . g:vimroam_global_vars.rxWikiInclSeparator
  let g:vimroam_global_vars.rxWikiInclSuffix1 = g:vimroam_global_vars.rxWikiInclArgs.
        \ g:vimroam_global_vars.rxWikiInclSuffix
  ...
  let g:vimroam_global_vars.hcolor_guifg_light = ['#aa5858', '#507030', '#1030a0', '#103040'
        \ , '#505050', '#636363']
  let g:vimroam_global_vars.hcolor_ctermfg_light = ['DarkRed', 'DarkGreen', 'DarkBlue', 'Black'
        \ , 'Black', 'Black']
  let g:vimroam_global_vars.hcolor_guifg_dark = ['#e08090', '#80e090', '#6090e0', '#c0c0f0'
        \ , '#e0e0f0', '#f0f0f0']
  let g:vimroam_global_vars.hcolor_ctermfg_dark = ['Red', 'Green', 'Blue', 'White', 'White'
        \ , 'White']
endfunction
...
function! s:extend_global(output_dic, default_dic) abort
  " Note: user_dic is unused here because it comes from g:vimroam_* vars
  " Copy the user's settings from variables of the form g:vimroam_<option> into the dict
  " g:vimroam_global_vars (or set a default value)
  for key in keys(a:default_dic)
    let value_infos = a:default_dic[key]
    if exists('g:vimroam_'.key)
      let user_value = g:vimroam_{key}
...
function! s:read_global_settings_from_user() abort
  ...
  for month in range(1, 12)
    if !has_key(users_value, month) || type(users_value[month]) != type('') ||
      ...
      call vimroam#u#error(printf('The provided value ''%s'' of the option ''g:vimroam_%s'' is'
            \ . ' invalid. See '':h g:vimroam_%s''.', string(users_value), key, key))
      break
    endif
  endfor
...
function! s:normalize_global_settings() abort
  let keys = keys(g:vimroam_global_vars.ext2syntax)
  for ext in keys
    " for convenience, we also allow the term 'mediawiki'
    if g:vimroam_global_vars.ext2syntax[ext] ==# 'mediawiki'
      let g:vimroam_global_vars.ext2syntax[ext] = 'media'
    endif
    ...
    if ext[0] !=# '.'
      let new_ext = '.' . ext
      let g:vimroam_global_vars.ext2syntax[new_ext] = g:vimroam_global_vars.ext2syntax[ext]
      call remove(g:vimroam_global_vars.ext2syntax, ext)
    endif
  endfor

  " ensure key_mappings dictionary has all required keys
  if !has_key(g:vimroam_global_vars.key_mappings, 'all_maps')
    let g:vimroam_global_vars.key_mappings.all_maps = 1
  endif
  if !has_key(g:vimroam_global_vars.key_mappings, 'global')
    let g:vimroam_global_vars.key_mappings.global = 1
  endif
  if !has_key(g:vimroam_global_vars.key_mappings, 'headers')
    let g:vimroam_global_vars.key_mappings.headers = 1
  endif
  if !has_key(g:vimroam_global_vars.key_mappings, 'text_objs')
    let g:vimroam_global_vars.key_mappings.text_objs = 1
  endif
  if !has_key(g:vimroam_global_vars.key_mappings, 'table_format')
    let g:vimroam_global_vars.key_mappings.table_format = 1
  endif
  if !has_key(g:vimroam_global_vars.key_mappings, 'table_mappings')
    let g:vimroam_global_vars.key_mappings.table_mappings = 1
  endif
  if !has_key(g:vimroam_global_vars.key_mappings, 'lists')
    let g:vimroam_global_vars.key_mappings.lists = 1
  endif
  if !has_key(g:vimroam_global_vars.key_mappings, 'links')
    let g:vimroam_global_vars.key_mappings.links = 1
  endif
  if !has_key(g:vimroam_global_vars.key_mappings, 'html')
    let g:vimroam_global_vars.key_mappings.html = 1
  endif
  if !has_key(g:vimroam_global_vars.key_mappings, 'mouse')
    let g:vimroam_global_vars.key_mappings.mouse = 0
  endif

  " disable all key mappings if all_maps == 0
  if !g:vimroam_global_vars.key_mappings.all_maps
    let g:vimroam_global_vars.key_mappings.global = 0
    let g:vimroam_global_vars.key_mappings.headers = 0
    let g:vimroam_global_vars.key_mappings.text_objs = 0
    let g:vimroam_global_vars.key_mappings.table_format = 0
    let g:vimroam_global_vars.key_mappings.table_mappings = 0
    let g:vimroam_global_vars.table_mappings = 0 " kept for backwards compatibility
    let g:vimroam_global_vars.key_mappings.lists = 0
    let g:vimroam_global_vars.key_mappings.links = 0
    let g:vimroam_global_vars.key_mappings.html = 0
    let g:vimroam_global_vars.key_mappings.mouse = 0
    let g:vimroam_global_vars.use_mouse = 0 " kept for backwards compatibility
  endif
  ...
  if g:vimroam_global_vars.table_mappings == 0 && g:vimroam_global_vars.key_mappings.table_mappings == 0
    let g:vimroam_global_vars.key_mappings.table_mappings = 0
  endif
  if g:vimroam_global_vars.use_mouse == 1 && g:vimroam_global_vars.key_mappings.mouse == 0
    let g:vimroam_global_vars.key_mappings.mouse = 1
  endif
endfunction
...
function! s:get_default_wikilocal() abort
  ...
  " Must be coherent with VimRoamColorize
  ...
  return {
        ...
        \ 'custom_wiki2html': {'type': type(''), 'default': 'vimwiki_markdown'},
        ...
        \ 'ext': {'type': type(''), 'default': '.md', 'min_length': 1},
        \ 'bullet_types': {'type': type([]), 'default': []},
        \ 'cycle_bullets': {'type': type(0), 'default': 0},
        \ 'html_filename_parameterization': {'type': type(0), 'default': 1, 'min': 0, 'max': 1},
        ...
        \ 'listsym_rejected': {'type': type(''), 'default': vimroam#vars#get_global('listsym_rejected')},
        \ 'listsyms': {'type': type(''), 'default': vimroam#vars#get_global('listsyms')},
...
function! s:populate_wikilocal_options() abort
  ...
  " Extend from g:vimroam_list
  if !exists('g:vimroam_list')
    ...
    call add(g:vimroam_wikilocal_vars, new_wiki_dic)
  else
    for user_dic in g:vimroam_list
  ...
  call add(g:vimroam_wikilocal_vars, temp_dic)
...
function! s:normalize_wikilocal_settings() abort
  for wiki_settings in g:vimroam_wikilocal_vars
    ...
    if !has_key(wiki_settings, 'bullet_types') || len(wiki_settings.bullet_types) == 0
      let wiki_settings.bullet_types = vimroam#vars#get_syntaxlocal('bullet_types', wiki_settings.syntax)
    endif
    ...
    for keyword in keys(wiki_settings.nested_syntaxes)
      if type(keyword) != type('') || empty(keyword) || type(wiki_settings.nested_syntaxes[keyword]) != type('') ||
        ...
        call vimroam#u#error(printf('The provided value ''%s'' of the option ''g:vimroam_%s'' is'
              \ . ' invalid. See '':h g:vimroam_%s''.', string(wiki_settings.nested_syntaxes), 'nested_syntaxes', 'nested_syntaxes'))
        break
      endif
    endfor
...
" Helper path
" TODO move to path: Conflict with: vimroam#path#path_norm && vimroam#path#normalize
...
function! s:get_default_syntaxlocal() abort
  ...
  return extend(s:get_common_syntaxlocal(), {
        ...
        \   'bold': vimroam#u#hi_expand_regex([['\*', '\*']]),
        \   'italic': vimroam#u#hi_expand_regex([['_', '_']]),
        \   'underline': vimroam#u#hi_expand_regex([]),
        \   'bold_italic': vimroam#u#hi_expand_regex([['\*_', '_\*'], ['_\*', '\*_']]),
        \   'code': [
        \       ['\%(^\|[^`]\)\@<=`\%($\|[^`]\)\@=',
        \        '\%(^\|[^`]\)\@<=`\%($\|[^`]\)\@='],
        \       ['\%(^\|[^`]\)\@<=``\%($\|[^`]\)\@=',
...
function! s:get_markdown_syntaxlocal() abort
  ...
  return extend(s:get_common_syntaxlocal(), {
        ...
        \ 'typeface': {'type': type({}), 'default': {
        \   'bold': vimroam#u#hi_expand_regex([['__', '__'], ['\*\*', '\*\*']]),
        \   'italic': vimroam#u#hi_expand_regex([['\*', '\*'], ['_', '_']]),
        \   'underline': vimroam#u#hi_expand_regex([]),
        \   'bold_italic': vimroam#u#hi_expand_regex([['\*_', '_\*'], ['_\*', '\*_'], ['\*\*\*', '\*\*\*'], ['___', '___']]),
        \   'code': [
        \       ['\%(^\|[^`]\)\@<=`\%($\|[^`]\)\@=',
        \        '\%(^\|[^`]\)\@<=`\%($\|[^`]\)\@='],
        \       ['\%(^\|[^`]\)\@<=``\%($\|[^`]\)\@=',
...
function! s:get_common_syntaxlocal() abort
  let res = {}
  let res.nested_extended = {'type': type(''), 'default': 'VimRoamError,VimRoamPre,VimRoamCode,VimRoamEqIn,VimRoamSuperScript,VimRoamSubScript,textSnipTEX'}
  let res.nested_typeface = {'type': type(''), 'default': 'VimRoamBold,VimRoamItalic,VimRoamUmderline,VimRoamDelText'}
...
" Exported: syntax/vimroam.vim
function! vimroam#vars#populate_syntax_vars(syntax) abort
  " TODO refactor <= too big function
  " TODO permit user conf in some var like g:vimroam_syntaxlocal_vars
  " TODO internalize match and search (header and bold)
  " Create is not exists
  if !exists('g:vimroam_syntaxlocal_vars')
    let g:vimroam_syntaxlocal_vars = {}
  endif

  " Clause: leave if already filled
  if has_key(g:vimroam_syntaxlocal_vars, a:syntax)
    return
  endif

  " Init internal dic
  let g:vimroam_syntaxlocal_vars[a:syntax] = {}
  let syntax_dic = g:vimroam_syntaxlocal_vars[a:syntax]
  ...
  let wikilocal = g:vimroam_wikilocal_vars[vimroam#vars#get_bufferlocal('wiki_nr')]
  " TODO remake tests
  "call s:extend_local(syntax_dic, default_dic, syntax_dic, wikilocal)
  " Extend <- user syntaxlocal
  if exists('g:vimroam_syntax_list') && has_key(g:vimroam_syntax_list, a:syntax)
    call s:extend_local(syntax_dic, default_dic, syntax_dic, g:vimroam_syntax_list[a:syntax])
  endif

  " TODO make that clean (i.e clearify what is local to syntax ot to buffer)
  " Get from local vars
  let bullet_types = vimroam#vars#get_wikilocal('bullet_types')
  if !empty(bullet_types)
    let syntax_dic['bullet_types'] = bullet_types
  endif
  let syntax_dic['cycle_bullets'] =
        \ vimroam#vars#get_wikilocal('cycle_bullets')

  " Tag: get var
  " TODO rename for internal
  let syntax_dic.tag_format = {}
  let tf = syntax_dic.tag_format
  call extend(tf, vimroam#vars#get_wikilocal('tag_format'))
  ...
  " Used: vimroam#base#get_anchors <- GenerateTagLinks
  ...
  for i in syntax_dic.number_types
    ...
    let syntax_dic.number_divisors .= vimroam#u#escape(i[1])
  endfor
  ...
  if !empty(syntax_dic.number_types)
    ...
    for type in syntax_dic.number_types[:-2]
      let syntax_dic.rxListNumber .= char_to_rx[type[0]] .
            \ vimroam#u#escape(type[1]) . '\|'
    endfor
    let syntax_dic.rxListNumber .=
          \ char_to_rx[syntax_dic.number_types[-1][0]].
          \ vimroam#u#escape(syntax_dic.number_types[-1][1]) . '\)'
  else
    "regex that matches nothing
    let syntax_dic.rxListNumber = '$^'
  endif
  ...
  " let g:vimroam_rxWeblink = '[\["(|]\@<!'. g:vimroam_rxWeblinkUrl .
  " \ '\%([),:;.!?]\=\%([ \t]\|$\)\)\@='
  let syntax_dic.rxWeblink =
        \ '\<'. g:vimroam_global_vars.rxWeblinkUrl . '[^[:space:]><]*'
  ...
  let syntax_dic.WikiLinkMatchUrlTemplate =
        \ g:vimroam_global_vars.rx_wikilink_prefix .
        \ '\zs__LinkUrl__\ze\%(#.*\)\?' .
        \ g:vimroam_global_vars.rx_wikilink_suffix .
        \ '\|' .
        \ g:vimroam_global_vars.rx_wikilink_prefix .
        \ '\zs__LinkUrl__\ze\%(#.*\)\?' .
        \ g:vimroam_global_vars.rx_wikilink_separator .
        \ '.*' .
        \ g:vimroam_global_vars.rx_wikilink_suffix

  " a) match [[URL|DESCRIPTION]]
  let syntax_dic.rxWikiLink = g:vimroam_global_vars.rx_wikilink_prefix.
        \ g:vimroam_global_vars.rxWikiLinkUrl.'\%('.g:vimroam_global_vars.rx_wikilink_separator.
        \ g:vimroam_global_vars.rxWikiLinkDescr.'\)\?'.g:vimroam_global_vars.rx_wikilink_suffix
  let syntax_dic.rxAnyLink =
        \ syntax_dic.rxWikiLink.'\|'.
        \ g:vimroam_global_vars.rxWikiIncl.'\|'.syntax_dic.rxWeblink
  " b) match URL within [[URL|DESCRIPTION]]
  let syntax_dic.rxWikiLinkMatchUrl =
        \ g:vimroam_global_vars.rx_wikilink_prefix . '\zs'. g:vimroam_global_vars.rxWikiLinkUrl
        \ .'\ze\%('. g:vimroam_global_vars.rx_wikilink_separator
        \ . g:vimroam_global_vars.rxWikiLinkDescr.'\)\?'.g:vimroam_global_vars.rx_wikilink_suffix
  " c) match DESCRIPTION within [[URL|DESCRIPTION]]
  let syntax_dic.rxWikiLinkMatchDescr =
        \ g:vimroam_global_vars.rx_wikilink_prefix . g:vimroam_global_vars.rxWikiLinkUrl
        \ . g:vimroam_global_vars.rx_wikilink_separator.'\%(\zs'
        \ . g:vimroam_global_vars.rxWikiLinkDescr. '\ze\)\?'
        \ . g:vimroam_global_vars.rx_wikilink_suffix
...
function! s:populate_list_vars(wiki) abort
  ...
  let recurring_bullets = vimroam#vars#get_syntaxlocal('recurring_bullets')
  let rxListNumber = vimroam#vars#get_syntaxlocal('rxListNumber')
  ...
  " the user can set the listsyms as string, but vimroam needs a list
  ...
  if match(a:wiki.listsyms, '[' . a:wiki.listsym_rejected . ']') != -1
    call vimroam#u#warn('the value of listsym_rejected ('''
...
function! s:populate_extra_markdown_vars() abort
  let mkd_syntax = g:vimroam_syntaxlocal_vars['markdown']
  ...
  let rx_wikilink_md_separator = vimroam#u#escape(wikilink_md_separator)
  let mkd_syntax.rx_wikilink_md_prefix = vimroam#u#escape(wikilink_md_prefix)
  let mkd_syntax.rx_wikilink_md_suffix = vimroam#u#escape(wikilink_md_suffix)
  ...
  let rxWeblink1Ext = ''
  if vimroam#vars#get_wikilocal('markdown_link_ext')
  ...
  let mkd_syntax.rxWeblink1Prefix = vimroam#u#escape(mkd_syntax.rxWeblink1Prefix)
  let mkd_syntax.rxWeblink1Separator = vimroam#u#escape(mkd_syntax.rxWeblink1Separator)
  ...
  let mkd_syntax.rxAnyLink = mkd_syntax.rxWikiLink.'\|'.
        \ g:vimroam_global_vars.rxWikiIncl.'\|'.mkd_syntax.rxWeblink .'\|'.
        \ mkd_syntax.rxImage

  let mkd_syntax.rxMkdRef = '\['.g:vimroam_global_vars.rxWikiLinkDescr.']:\%(\s\+\|\n\)'.
        \ mkd_syntax.rxWeblink0
  let mkd_syntax.rxMkdRefMatchDescr =
        \ '\[\zs'.g:vimroam_global_vars.rxWikiLinkDescr.'\ze]:\%(\s\+\|\n\)'. mkd_syntax.rxWeblink0
  let mkd_syntax.rxMkdRefMatchUrl =
        \ '\['.g:vimroam_global_vars.rxWikiLinkDescr.']:\%(\s\+\|\n\)\zs'.
...
function! s:normalize_syntax_settings(syntax) abort
  ...
  if a:syntax ==# 'markdown'
    ...
    let syntax_dic.Link1 = vimroam#vars#get_global('WikiLinkTemplate1')
  endif
endfunction
...
function! s:get_anywhere(key, ...) abort
  " Alias common info
  let s:syntax = vimroam#vars#get_wikilocal('syntax')
  let s:wiki_nr = vimroam#vars#get_bufferlocal('wiki_nr')
  let s:wikilocal = g:vimroam_wikilocal_vars[s:wiki_nr]
  let s:user_wiki = get(g:vimroam_list, s:wiki_nr, {})
  ...
  function! s:any_bufferlocal(key, value) abort
    if !(v:version > 703 && exists('b:vimroam_'.a:key)) | return | endif
    exe 'let b:vimroam_' . a:key . ' = ' . a:1
  endfunction
  ...
  function! s:any_syntaxlocal(key, value) abort
    ...
    unlet g:vimroam_syntaxlocal_vars

    " Build vars
    call vimroam#vars#populate_syntax_vars(s:syntax)

    " Update syntax
    syntax clear
    runtime syntax/vimroam.vim
    redraw
  endfunction
  ...
  function! s:any_global(key, value) abort
    "let g:vimroam_global_vars[a:key] = a:1
    exe 'let g:vimroam_' . a:key . ' = ' . string(a:value)
    call s:populate_global_variables()
  endfunction
  ...
  if has_key(s:get_default_global(), a:key)
    ...
    return [g:vimroam_global_vars[a:key], 'global']
  ...
  " -- Syntax Local
  elseif has_key(g:vimroam_syntaxlocal_vars[s:syntax], a:key)
    if a:0
      call s:any_wikilocal(a:key, value)
      call s:any_syntaxlocal(a:key, value)
      call s:any_bufferlocal(a:key, value)
    endif
    return [g:vimroam_syntaxlocal_vars[s:syntax][a:key], 'syntaxlocal']

  " -- Buffer Local
  elseif v:version > 703 && exists('b:vimroam_'.a:key)
    if a:0
      call s:any_bufferlocal(a:key, value)
    endif
    return [get(getbufvar('%', ''), 'vimroam_'.a:key, '/\/\'), 'bufferlocal']
  else
    return ['', 'none']
  endif
endfunction
...
function! vimroam#vars#cmd(arg) abort
  ...
  if arg_key ==# ''
    ...
    let syntax = vimroam#vars#get_wikilocal('syntax')
    let d_slocal = function('s:get_' . syntax . '_syntaxlocal')()
    let d_default = {}

    " Define helpers
    function! s:print_head(name) abort
      call vimroam#u#echo(repeat('-', 50), 'Statement', '', '')
      call vimroam#u#echo('  ' . a:name, 'Statement', '', '')
      call vimroam#u#echo(repeat('-', 50), 'Statement', '', '')
    endfunction
    ...
    for key in sort(keys(d_global))
      if !has_key(g:vimroam_global_vars, key)
        continue
      endif
      if string(g:vimroam_global_vars[key]) == string(d_global[key].default)
        let d_default[key] = string(d_global[key].default) . '          " From Global'
      else
        let msg = key .  ' = ' . string(g:vimroam_global_vars[key])
        call vimroam#u#echo(msg, '', 'm', '')
      endif
    endfor

    " Print SyntaxLocal
    let syntaxlocal = g:vimroam_syntaxlocal_vars[syntax]
    ...
    let wikilocal = g:vimroam_wikilocal_vars[wiki_nr]
...
function! vimroam#vars#complete(arglead, cmdline, pos) abort
...
function! vimroam#vars#get_syntaxlocal(key, ...) abort
  ...
  if !exists('g:vimroam_syntaxlocal_vars') || !has_key(g:vimroam_syntaxlocal_vars, syntax)
    call vimroam#vars#populate_syntax_vars(syntax)
  endif

  " Return d_syntax[a:key]
  return g:vimroam_syntaxlocal_vars[syntax][a:key]
endfunction
...
function! vimroam#vars#get_bufferlocal(key, ...) abort
  let buffer = a:0 ? a:1 : '%'

  " 'get(getbufvar(...' handles vim < v7.3.831 that didn't allow a default value for getbufvar
  let value = get(getbufvar(buffer, ''), 'vimroam_'.a:key, '/\/\')
  if type(value) != 1 || value !=# '/\/\'
    return value
  elseif a:key ==# 'wiki_nr'
    call setbufvar(buffer, 'vimroam_wiki_nr', vimroam#base#find_wiki(expand('%:p')))
  elseif a:key ==# 'subdir'
    call setbufvar(buffer, 'vimroam_subdir', vimroam#base#current_subdir())
  elseif a:key ==# 'invsubdir'
    let subdir = vimroam#vars#get_bufferlocal('subdir')
    call setbufvar(buffer, 'vimroam_invsubdir', vimroam#base#invsubdir(subdir))
  elseif a:key ==# 'existing_wikifiles'
    call setbufvar(buffer, 'vimroam_existing_wikifiles',
          \ vimroam#base#get_wikilinks(vimroam#vars#get_bufferlocal('wiki_nr'), 1, ''))
  elseif a:key ==# 'existing_wikidirs'
    call setbufvar(buffer, 'vimroam_existing_wikidirs',
        \ vimroam#base#get_wiki_directories(vimroam#vars#get_bufferlocal('wiki_nr')))
  elseif a:key ==# 'prev_links'
    call setbufvar(buffer, 'vimroam_prev_links', [])
  elseif a:key ==# 'markdown_refs'
    call setbufvar(buffer, 'vimroam_markdown_refs', vimroam#markdown_base#scan_reflinks())
  else
    call vimroam#u#echo('unknown buffer variable ' . string(a:key))
  endif

  return getbufvar(buffer, 'vimroam_'.a:key)
endfunction


" Set buffer local variable
function! vimroam#vars#set_bufferlocal(key, value, ...) abort
  let buffer = a:0 ? a:1 : '%'
  call setbufvar(buffer, 'vimroam_' . a:key, a:value)
endfunction


" Return: wiki global variable
function! vimroam#vars#get_global(key) abort
  return g:vimroam_global_vars[a:key]
endfunction


" Set global variable
function! vimroam#vars#set_global(key, value) abort
  let g:vimroam_global_vars[a:key] = a:value
  return g:vimroam_global_vars[a:key]
endfunction
...
" used
function! vimroam#vars#get_wikilocal(key, ...) abort
  if a:0
    return g:vimroam_wikilocal_vars[a:1][a:key]
  else
    return g:vimroam_wikilocal_vars[vimroam#vars#get_bufferlocal('wiki_nr')][a:key]
  endif
endfunction


" Return: wiki local variable (of default wiki [index -1])
function! vimroam#vars#get_wikilocal_default(key) abort
  return g:vimroam_wikilocal_vars[-1][a:key]
endfunction
...
" Param: (2): wiki number (<int>). When absent, the wiki of the currently active buffer is
" used
function! vimroam#vars#set_wikilocal(key, value, ...) abort
  if a:0
    let wiki_nr = a:1
  else
    let wiki_nr = vimroam#vars#get_bufferlocal('wiki_nr')
  endif
  if wiki_nr == len(g:vimroam_wikilocal_vars) - 1
    call insert(g:vimroam_wikilocal_vars, {}, -1)
  endif
  let g:vimroam_wikilocal_vars[wiki_nr][a:key] = a:value
endfunction


" Append new wiki to wiki list
function! vimroam#vars#add_temporary_wiki(settings) abort
  let new_temp_wiki_settings = copy(g:vimroam_wikilocal_vars[-1])
  ...
  call insert(g:vimroam_wikilocal_vars, new_temp_wiki_settings, -1)
  call s:normalize_wikilocal_settings()
endfunction


" Return: number of registered wikis + temporary
function! vimroam#vars#number_of_wikis() abort
  return len(g:vimroam_wikilocal_vars) - 1
endfunction
